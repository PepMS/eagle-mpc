...
boost::shared_ptr<crocoddyl::CostModelSum> CarrotMpc::createCosts() const
{
    ...
    boost::shared_ptr<crocoddyl::ResidualModelState> state_reg_residual =
        boost::make_shared<crocoddyl::ResidualModelState>(robot_state_, robot_state_->zero(), actuation_->get_nu());
    boost::shared_ptr<crocoddyl::CostModelResidual> state_reg_cost =
        boost::make_shared<crocoddyl::CostModelResidual>(robot_state_, state_activation, state_reg_residual);
    ...
    boost::shared_ptr<crocoddyl::CostModelResidual> control_cost =
        boost::make_shared<crocoddyl::CostModelResidual>(robot_state_, control_activation, control_residual);
    ...
    boost::shared_ptr<crocoddyl::ResidualModelState> carrot_residual =
        boost::make_shared<crocoddyl::ResidualModelState>(robot_state_, robot_state_->zero(), actuation_->get_nu());
    boost::shared_ptr<crocoddyl::CostModelResidual> carrot_cost =
        boost::make_shared<crocoddyl::CostModelResidual>(robot_state_, carrot_residual);
    ...
    boost::shared_ptr<crocoddyl::ResidualModelState> carrot_tail_residual =
        boost::make_shared<crocoddyl::ResidualModelState>(robot_state_, robot_state_->zero(), actuation_->get_nu());
    boost::shared_ptr<crocoddyl::CostModelResidual> carrot_tail_cost =
        boost::make_shared<crocoddyl::CostModelResidual>(robot_state_, activation, carrot_tail_residual);
...
void CarrotMpc::updateFreeCosts(const std::size_t& idx, const std::size_t& current_time)
{
    ...
    if (update_vars_.idx_stage < trajectory_->get_stages().size()) {
        if (!trajectory_->get_stages()[update_vars_.idx_stage]->get_is_transition() ||
            (idx == dif_models_.size() - 1)) {
            update_vars_.dif_free->get_costs()->get_costs().at("carrot_state")->active = true;
            computeStateReference(update_vars_.node_time);
            // update_vars_.dif_free->get_costs()
            //     ->get_costs()
            //     .at("carrot_state")
            //     ->cost->set_reference(update_vars_.state_ref);
            boost::static_pointer_cast<crocoddyl::ResidualModelState>(
                update_vars_.dif_free->get_costs()->get_costs().at("carrot_state")->cost->get_residual())
        ...
        boost::static_pointer_cast<crocoddyl::ResidualModelState>(
            update_vars_.dif_free->get_costs()->get_costs().at("carrot_tail")->cost->get_residual())
            ->set_reference(update_vars_.state_ref);
    }
}
