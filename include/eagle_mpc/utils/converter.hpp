    ...
    static utils::list<A> convert(std::string val)
    {
        ...
        if (std::regex_match(val, rgxP)) {
            ...
            throw std::runtime_error(
                "Invalid string format representing a list-like structure. Correct format is [(value)?(,value)*]. "
                "String "
                "provided: " +
                val);
        return result;
...
struct converter<bool> {
    static bool convert(std::string val)
    {
        if (val == "true")
            ...
            throw std::runtime_error(
                "Invalid conversion to bool (Must be either \"true\" or \"false\"). String provided: " + val);
    }
};
...
struct converter<std::string> {
    ...
    {
        // throw std::runtime_error("There is no general convert to string for arbitrary T !!! String provided: " +
        // val);
...
struct converter<std::pair<A, B>> {
    static std::pair<A, B> convert(std::string val)
    {
        ...
        if (std::regex_match(val, matches, rgxP)) {
            ...
            throw std::runtime_error(
                "Invalid string format representing a pair. Correct format is {identifier:value}. String provided: " +
                val);
    }
};
